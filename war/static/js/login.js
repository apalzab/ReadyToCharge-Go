if ($.cookie('ReadyToChargeAndGo') != null) {
    ...
    var id = ret[0];
    var pass = ret[1];
    $('form.login input[type="text"]').hide();
    $('form.login input[type="password"]').hide();
    $('form.login button:eq(1)').hide();
    loginUser(id, pass);
  } else {
      $('form.login input[type="text"]').show();
  ...
  $('nav button:eq(0)').click(function(e) {
    e.preventDefault();
    user = $('form.login input[type="text"]').val();
  ...
  $('form.login .logOut').click(function() {
    ...
    $('form.login input[type="text"]').val("").show('slow');
  ...
  function progressBarStart(){
    $('.progressBar').remove();
    $('#nav').after('<div class="progressBar"><div class="progress progress-striped active title"><div class="bar" ></div></div><p align="center">Solicitando datos....contactando con el servidor........esto puede durar un tiempo....</p></div>');
      var cont=0;
      for (i=0; i<3000; i++)
        {
